...
function leaguewp_setup() {
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'leaguewp' ),
	) );
 ...
 */
function leaguewp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'leaguewp_content_width', 640 );
}
add_action( 'after_setup_theme', 'leaguewp_content_width', 0 );
...
function leaguewp_scripts() {
	wp_enqueue_style( 'leaguewp-style', get_stylesheet_uri() );

	wp_enqueue_script( 'leaguewp-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'leaguewp-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
